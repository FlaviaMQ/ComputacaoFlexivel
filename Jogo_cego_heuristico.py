...
ALTURA = LINHAS * TAMANHO_CELULA + 150
...
def gerar_malha():
    return [[random.randint(10, 80) for _ in range(COLUNAS)] for _ in range(LINHAS)]
...
def desenhar_painel(tela, fonte, modo, passo, total_passos, area_irrigar, custo_busca):
    y = LINHAS * TAMANHO_CELULA

    pygame.draw.rect(tela, CINZA_CLARO, (0, y, LARGURA, 150))
...
def calcular_custo(modo, passo):
    if modo == "Busca cega":
        return f"{passo} avaliacoes sequenciais"

    elif modo == "Busca heuristica":
        return f"{passo} avaliacoes + ordenacao por umidade"
...
def main():
    ...
    while executando:
        ...
        custo_busca = calcular_custo(modo, passo)
